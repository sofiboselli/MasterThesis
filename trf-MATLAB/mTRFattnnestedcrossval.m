function [sts,sts2] = mTRFattnnestedcrossval(stim,stim2,resp,fs,Dir,tmin,tmax,lambdas,verbose,zeropad)
...
for n = 1:nfold
    ...
    if length(lambdas)==1 % if only one lambdas value is specified
        ...
        % use mTRFtrain
        model = mTRFtrain(stim(train_tr),resp(train_tr),fs,Dir,tmin,tmax,lambdas,'verbose',0);
        
    else % otherwise, use mTRFcrossval
        
        %[cvr,~,~,cvw,t,cvb] = mTRFcrossval(stim(train_tr),resp(train_tr),fs,Dir,tmin,tmax,lambdas,[]);
        stats = mTRFcrossval(stim(train_tr),resp(train_tr),fs,Dir,tmin,tmax,lambdas,'verbose',0);
...
%         w{n} = cvw(:,:,:,opt_idx);
%         b{n} = cvb(:,:,opt_idx);
        model = mTRFtrain(stim(train_tr),resp(train_tr),fs,Dir,tmin,tmax,opt_lambda(n),'verbose',0);
    ...
    % testing
    [~,st] = mTRFpredict(stim(tst_tr),resp(tst_tr),model,'verbose',0);
    sts(n) = st.r;
    [~,st2] = mTRFpredict(stim2(tst_tr),resp(tst_tr),model,'verbose',0);
    sts2(n) = st2.r;
